...
if($statusnumrows > 0){
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		if ($avatar_pm == ""){
			if ($fbPm != 'Null')
			{
				$img_src_pm = "http://graph.facebook.com/$fbPm/picture?type=large";
			}elseif ($row["gender"] == 'f')
				{
		...
		$mail .= '<button class="notificatioSectionMsgMarkAsReadBtn" onclick="markRead(\''.$pmid.'\',\''.$sender.'\',\''.$userName.'\',\'mark_as_read\')">Mark As Read</button>';
		// Add Delete button
		$mail .= '<button class="notificatioSectionMsgDeleteMsgBtn" id="'.$btid2.'" onclick="deletePm(\''.$pmid.'\',\''.$wrap.'\',\''.$wrap2.'\',\''.$sender.'\',\''.$userName.'\',\'delete_pm\')">Delete</button></div>';
    	...
    	if($replynumrows > 0){
			while ($row2 = mysqli_fetch_array($query_replies, MYSQLI_ASSOC)) {
				...
				$mail .= '<div class ="pm_post"><label class="notificatioSectionMsgReplyLabel">Reply From:</label>
		...
		$mail .= '</div>';
		// Add reply textbox
		$mail .= '</div><div  id="'.$wrap2.'" class="pm_wrap1"><div id="notificatioSectionTextAreaBox"><textarea class="notificatioSectionTextArea" id="'.$rt.'" placeholder="Reply..." onclick="this.value=\'\' rows="4" cols="50""></textarea><br />';
		// Add reply button
		$mail .= '<button class="notificatioSectionTextAreaBtn" id="'.$rb.'" onclick="replyToPm('.$pmid.',\''.$u.'\',\''.$rt.'\',\''.$rb.'\',\''.$sender.'\')">Reply</button>';
		$mail .= '</div></div>';
	}
}
?>
